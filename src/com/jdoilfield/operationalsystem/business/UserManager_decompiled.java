...
public class UserManager_decompiled implements ManagerInterface<User> {
	...
	public String add(User u) {
		...
		try {
			if (!(this.userDao.existUser(u))) {
				...
				if ((u.getIdSupervisor() != null)
						&& (u.getIdSupervisor().intValue() == 0)) {
				...
				this.logger.info("Se ha insertado el usuario: " + u.getLogin());
			}
	...
	public String update(User u) {
		// Check if the operations comes from the Recovery Password Htm
		if (u.getSend_crendentials().equals("send_credentials")) {
			logger.info("LETS SEND THE NEW CREDENTIALS.");
		}
...

		{
			if (!(this.userDao.existUser(u))) {
				...
				if ((u.getNewPassword1() != null)
						&& (!(u.getNewPassword1().isEmpty())))
				...
				this.logger
						.info("Se ha modificado el usuario: " + u.getLogin());
			}
	...
	public String delete(String id) {
		try {
			...
			this.logger.error(
					"Ocurrio un error al intentar eliminar el usuario: " + id,
					e);
	...
	public User getElement(String id) throws LogicException {
		...
		try {
			user = (User) this.userDao.findById(Integer.valueOf(Integer
					.parseInt(id)));
		} catch (PersistenceException e) {
			this.logger.error(
					"Ocurrio un error al intentar consultar el usuario: " + id,
					e);
	...
	public ResultList getResultList(Search s, int page) throws LogicException {
		...
		try {
			userList = this.userDao.find(s,
					Integer.valueOf((page - 1) * s.getNumRows()));
		} catch (PersistenceException e) {
			this.logger
					.error("Ocurrio un error al intentar realizar la busqueda de los usuarios",
							e);
			throw new LogicException(e.getMessage(), e);
		} catch (Exception e) {
			this.logger
	...
	public List<User> getList() throws LogicException {
		...
		try {
			...
			this.logger
					.error("Ocurrio un error al intentar obtener la lista de usuarios",
							e);
			throw new LogicException(e.getMessage(), e);
		}
		return list;
	...
	public Hashtable<String, Object> loadLists() throws LogicException {
		...
		try {
			...
			this.logger
					.error("Ocurrio un error al intentar obtener las listas de los combos de la vista ingresar o modificar Usuario ",
							e);
			throw new LogicException(e.getMessage(), e);
		}
		return data;
	...
	public Hashtable<String, Object> loadSearchList() throws LogicException {
		...
		try {
			...
			this.logger
					.error("Ocurrio un error al intentar obtener las listas de los combos de busqueda de Usuarios",
							e);
			throw new LogicException(e.getMessage(), e);
		}
		return data;
	...
	public String changePassword(User u, String password, String newpassword) {
		try {
			if (u.getPassword().equals(Utilities.MD5(password))) {
				this.userDao.changePassword(u.getId(),
						Utilities.MD5(newpassword));
				this.result = "success.update.password";
			}
			this.result = "errors.notmatch.password";
		} catch (PersistenceException e) {
			this.logger.error(
					"Ocurrio un error al intentar cambiar el password del usuario: "
							+ u.getId(), e);
	...
	public User findByUserName(String username) throws LogicException {
		...
		try {
			...
			this.logger.error(
					"Ocurrio un error al intentar obtener la informacion de usuario del login: "
							+ username, e);
		}
